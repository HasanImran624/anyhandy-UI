    ...
    <>
      {signUp && (
        <section className="w-screen h-screen flex items-center justify-center fixed top-0 left-0 z-10">
          <div
            className="hidden sm_desktop:block w-screen h-screen bg-black bg-opacity-20 absolute top-0 left-0 z-20"
      ...
      <nav
        ...
        style={{
          background:
            "linear-gradient(180deg, rgba(135,206,250,0.2)3%, rgba(135,206,250,0.1) 21%, rgba(255,255,255,1) 86%)",
        }}
      >
        <section>
          <img
          ...
          {isOpen && (
            <section className="w-screen h-screen fixed top-0 left-0 z-10">
              ...
              <div className="w-4/5 max-w-[300px] h-screen overflow-y-scroll bg-white absolute top-0 right-0 z-30 space-y-7 py-5 px-7">
                ...
                <div className="flex items-center justify-center flex-col gap-5">
                  <button
                    className="w-fit font-semibold font-Onest p-4 text-base text-black hover:text-[#00CF91] border border-transparent rounded-md hover:border-[#E1DFD7] transition-all ease-in-out duration-100 cursor-pointer"
                    onClick={() => {
                      setSignUp(!signUp);
                      setIsOpen(false);
                    }}
